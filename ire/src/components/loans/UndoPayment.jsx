...
const UndoPayment = (props) => {
	...
	//!
	//!
	//! just reverse the  the balance to be == + sum
	//!
	//!
	//!
	...
	// 		totalInterest: parseFloat(total
	...
	const submitHandler = (e, payment_id) => {
		...
		axios
			.put(
				`http://localhost:8000/api/Loan/update/status/undo/${id}/${payment_id}`
			)
			.then((res) => {
	...
	return (
		<div>
			<Button
				className="btn btn-danger mt-3 d-inline-flex"
				onClick={() => {
					setShow(true);
				}}
			>
